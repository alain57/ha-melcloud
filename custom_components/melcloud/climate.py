...
try:
    from homeassistant.components.climate.const import (SUPPORT_TARGET_TEMPERATURE, SUPPORT_FAN_MODE,
...
class MelCloudAuthentication:
    ...
    def is_login(self):
        return self._contextkey is not None
    ...
    def login(self):
        ...
        req = requests.post("https://app.melcloud.com/Mitsubishi.Wifi.Client/Login/ClientLogin",
                            data={"Email": self._email, "Password": self._password, "Language": self._language,
                                  "AppVersion": "1.15.3.0", "Persist": False})

        if req.status_code == 200:
            reply = req.json()
            if "ErrorId" in reply and reply["ErrorId"] is None:
...
class MelCloudDevice:
    ...
    def _refresh_device_info(self, recursive=0):
        ...
        req = requests.get("https://app.melcloud.com/Mitsubishi.Wifi.Client/Device/Get",
                           headers={'X-MitsContextKey': self._authentication.get_context_key()},
                           data={'id': self._deviceid, 'buildingID': self._buildingid})
    ...
    # Method that checks that json is filled
    # it will refresh it if the info is too old
    ...
    def apply(self, recursive=0):
        ...
        req = requests.post("https://app.melcloud.com/Mitsubishi.Wifi.Client/Device/SetAta",
                            headers={'X-MitsContextKey': self._authentication.get_context_key()}, data=self._json)
        if req.status_code == 200:
    ...
    # Method that checks if the json is valid, that it contains the key param_to_read.
    # If yes return the value of this key, otherwise return the default value
    def _read_or_default(self, param_to_read, default_value=0):
        if not self._is_json_valid() or param_to_read not in self._json:
            return default_value
        return self._json[param_to_read]
    ...
    def get_temperature(self):
        return self._read_or_default("SetTemperature")

    def get_room_temperature(self):
        if not self._is_json_valid():
            return 0
    ...
    def get_fan_speed_max(self):
        return self._read_or_default("NumberOfFanSpeeds")

    def get_fan_speed(self):  # 0 Auto, 1 to NumberOfFanSpeeds
        return self._read_or_default("SetFanSpeed")

    def get_vertical_swing_mode(self):  # 0 Auto, 1 to NumberOfVane, +1 Swing
        return self._read_or_default("VaneVertical")

    def get_horizontal_swing_mode(self):  # 0 Auto, 1 to NumberOfVane, +1 Swing
        return self._read_or_default("VaneHorizontal")

    def get_mode(self):
        return self._read_or_default("OperationMode", Mode.Auto)

    def is_power_on(self):  # Boolean
        return self._read_or_default("Power", False)

    def is_online(self):  # Boolean
        return self._read_or_default("Offline", False)

    # Method that override a json value on a specific key.
    # if json is not valid it will return False and log the error
    def set_if_info_is_valid(self, key, value, logger_content_name):
        if not self._is_json_valid():
            _LOGGER.error("Unable to set " + logger_content_name + ": " + str(value))
            return False
        self._json[key] = value
        return True

    def set_vertical_swing_mode(self, swing_mode):
        return self.set_if_info_is_valid("VaneVertical", swing_mode, "swing mode")

    def set_horizontal_swing_mode(self, swing_mode):
        return self.set_if_info_is_valid("VaneHorizontal", swing_mode, "swing mode")

    def set_temperature(self, temperature):
        return self.set_if_info_is_valid("SetTemperature", temperature, "temperature")

    def set_fan_speed(self, speed):  # 0 Auto, 1 to NumberOfFanSpeeds
        return self.set_if_info_is_valid("SetFanSpeed", speed, "fan speed")

    def set_mode(self, mode):
        return self.set_if_info_is_valid("OperationMode", mode, "mode")

    def power_on(self):
        return self.set_if_info_is_valid("Power", True, "power")

    def power_off(self):
        return self.set_if_info_is_valid("Power", False, "power")
# ---------------------------------------------------------------
...
class MelCloud:
    ...
    def get_devices_list(self, recursive=0):
        ...
        req = requests.get("https://app.melcloud.com/Mitsubishi.Wifi.Client/User/ListDevices",
                           headers={'X-MitsContextKey': self._authentication.get_context_key()})
        if req.status_code == 200:
            reply = req.json()
            ...
            for entry in reply:
                ...
                for device in entry["Structure"]["Devices"]:
                    devices.append(MelCloudDevice(device["DeviceID"], device["BuildingID"], device["DeviceName"],
                ...
                for floor in entry["Structure"]["Floors"]:
                    ...
                    for areas in floor["Areas"]:
                        for device in areas["Devices"]:
                            devices.append(
                                MelCloudDevice(device["DeviceID"], device["BuildingID"], device["DeviceName"],
                                               self._authentication))
...
class MelCloudClimate(ClimateDevice):
    ...
    def __init__(self, device):
        ...
        self._fan_list.append('Speed ' + str(self._device.get_fan_speed_max()) + " (Max)")
    ...
    @property
    def name(self):
        return "MELCloud " + self._device.get_friendly_name() + " (" + str(self._device.get_id()) + ")"
    ...
    def current_operation(self):
        ...
        elif self._device.get_mode() == Mode.Heat:
            return OPERATION_HEAT_STR
        elif self._device.get_mode() == Mode.Cool:
            return OPERATION_COOL_STR
        elif self._device.get_mode() == Mode.Dry:
            return OPERATION_DRY_STR
        elif self._device.get_mode() == Mode.Fan:
            return OPERATION_FAN_STR
        elif self._device.get_mode() == Mode.Auto:
    ...
    def operation_list(self):
        return [OPERATION_HEAT_STR, OPERATION_COOL_STR, OPERATION_DRY_STR, OPERATION_FAN_STR, OPERATION_AUTO_STR,
                OPERATION_OFF_STR]
    ...
    def current_fan_mode(self):
        if self._device.get() >= len(self._fan_list):
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    if not MelCloudAuthentication(email, password, language).login():
        _LOGGER.error("melcloud: Invalid Login/Password  !")
        return False

    melcloud = MelCloud(mcauth)

    device_list = []

    for a_device in melcloud.get_devices_list():
        _LOGGER.debug("melcloud: Adding new device: " + a_device.get_friendly_name())
        device_list.append(MelCloudClimate(a_device))
